...
p <- ggplot(csvlong_combined |> filter(k == "k"), aes(x = stack, y = value, fill = stack)) + 
...
ggsave(filename = paste0("plots/plot_all_datasets.png"), plot = p, width = 12, height = 7, dpi = 300)
print(p)


# Filter unique datasets
datasets <- unique(csvlong_combined$data)
for (dataset in datasets) {
  p <- ggplot(csvlong_combined |> filter(data == dataset, k == "k"), aes(x = stack, y = value, fill = stack)) + 
    geom_violin(alpha = 0.4) +  # Make violins slightly transparent
    geom_boxplot(width = 0.15, outlier.shape = NA, color = "black", alpha = 0.7) +  # Add boxplot
    # geom_jitter(width = 0.1, size = 1.5, aes(color = stack), alpha = 0.7) +
    # facet_grid(data ~ metric, scales = "free_y") +
    ...
    theme_classic() +
    theme(legend.position = "right", axis.text.x = element_text(angle = 45, hjust = 1))  +  
    ggtitle(paste("Comparison for Dataset: ", dataset))
  
  ggsave(filename = paste0("plots/plot_", dataset, ".png"), plot = p, width = 12, height = 7, dpi = 300)
  print(p)
}
```
