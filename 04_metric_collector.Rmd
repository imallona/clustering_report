...
pivot_longer(
    ...
    cols = c('k-1', 'k-2', 'k', 'k+1', 'k+2'),
    names_to = 'ks') %>%
    ggplot(aes(x=ks, y= value, color = data)) +
    geom_point() +
...
ggplot(fd, aes(x=method, y= k, color = data)) +
    geom_point() +
    facet_grid(~metric) +
    theme_bw() +
    theme(text=element_text(family="Arial",size=12))
...
```{r, fig.width = 30, fig.height = 30, eval = TRUE}

fd <- fd[order(fd$metric, fd$k,fd$data, fd$method),]
...
plot(fd[,c('k-1', 'k-2', 'k', 'k+1', 'k+2')],
     col = as.numeric(as.factor(fd$data)),
     pch = as.numeric(as.factor(fd$method)))

plot(fd[,c('k-1', 'k-2', 'k', 'k+1', 'k+2')],
     col = as.numeric(as.factor(fd$metric)),
     pch = as.numeric(as.factor(fd$data)))
